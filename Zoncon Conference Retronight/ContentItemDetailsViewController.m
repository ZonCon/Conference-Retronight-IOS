...
#import "ContentItemDetailsViewController.h"
#import "globals.h"
#import "DbHelper.h"
...
@interface ContentItemDetailsViewController ()

@end

@implementation ContentItemDetailsViewController
...
- (void)viewDidLoad {
    ...
    NSDictionary *cv = @{
                         DB_COL_TYPE: DB_RECORD_TYPE_ITEM_VIEWS,
                         DB_COL_TITLE: _idSrvStream,
                         DB_COL_SUBTITLE: _idSrvItem,
                         };
    NSMutableArray *arr = [[DbHelper getSharedInstance] retrieveRecords:cv];
    
    if(arr.count > 0) {
        
        cv = [arr objectAtIndex:0];
        NSString *views = [cv objectForKey:DB_COL_NAME];
        int viewsI = [views intValue];
        viewsI++;
        views = [NSString stringWithFormat:@"%d", viewsI];
        ...
        [[DbHelper getSharedInstance] updateRecord:cvNew whereRecord:cv];
    ...
    for(int i = 0; i < arr.count; i++) {
        ...
        for(int j = 0; j < arrItems.count; j++) {
            ...
            _labelViews = [[UILabel alloc] initWithFrame:CGRectMake(screenWidth - margin - 100, topY, 100, 30)];
            _labelViews.textAlignment = NSTextAlignmentRight;
            _labelViews.numberOfLines = 0;
            _labelViews.textColor = [UIColor blackColor];
            _labelViews.font = contentFont;
            _labelViews.lineBreakMode = NSLineBreakByWordWrapping;
            topY = (imageShare.frame.origin.y + 30 + margin);
            [myContentView addSubview:_labelViews];
            ...
            if(itemContent.length > 0) {
                ...
                labelContent.text = [NSString stringWithFormat:@"%@ - %@", _strTimestamp, itemContent];
...
- (void) populateViews {
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@%@", SERVER, API, API_GET_ITEM_VIEWS];
    NSString *myRequestString = [NSString stringWithFormat:@"params=[{\"idProject\": \"%@\", \"idStream\": \"%@\", \"idItem\": \"%@\"}]", PID, _idSrvStream, _idSrvItem];
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:[NSOperationQueue currentQueue] completionHandler:^(NSURLResponse *response, NSData *data, NSError *error)
     {
         ...
         if(error == nil) {
             ...
             NSString *result = [json objectForKey:@"value"];
             result = [result stringByReplacingOccurrencesOfString:@"\""
                                            withString:@""];
             _labelViews.text = [NSString stringWithFormat:@"%@ Views", result];

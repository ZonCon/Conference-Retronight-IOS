...
#import "ContentItemsListCollectionViewCell.h"
#import "ContentItemDetailsViewController.h"
#import "globals.h"
#import "DbHelper.h"
#import "NavigationViewController.h"
#import "SyncViewController.h"
#import "ContentItemsListFooterCollectionReusableView.h"

@interface ContentItemsListCollectionViewController ()

@end

@implementation ContentItemsListCollectionViewController
...
- (void)loadFromLocalDB {
    ...
    for(int i = 0; i < arr.count; i++) {
        ...
        int j = 0;
        if ([[title lowercaseString] rangeOfString:@"banner"].location == NSNotFound) {
            j = 1;
            OFFSET++;
        } else {
            
        }
        for(; j < arrItems.count; j++) {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([[segue identifier] isEqualToString:@"DesignsDetails"])
    {
        ...
        ContentItemDetailsViewController *vc = [segue destinationViewController];
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return CGSizeMake(width - 20, 82);
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ContentItemsListCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
    ...
    if(_arrTitles.count > 0 && _arrSubTitles.count > 0 && _arrTimes.count > 0) {
        ...
        cell.textSub.text = subTitle;
        cell.textTime.text = timestamp;
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ContentItemsListFooterCollectionReusableView *footerview = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:@"DesignsListFooter" forIndexPath:indexPath];
